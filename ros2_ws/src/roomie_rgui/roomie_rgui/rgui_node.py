...
class RobotGuiNode(Node):
    ...
    def handle_countdown_completed(self):
        ...
        # 현재 화면 상태를 확인하여 다음 화면 결정
        current_screen = self.screen.get_current_screen_name()
        
        if request.task_type_id in [0, 1]:  # 0: 음식배송, 1: 비품배송
            # 현재 화면에 따라 픽업 출발 vs 배송 출발 구분
            if current_screen in ["TOUCH_SCREEN", "COUNTDOWN", None]:
                # 첫 번째 카운트다운 = 픽업 출발
                ...
                self.get_logger().warn(f"배송 카운트다운이지만 예상치 못한 현재 화면: {current_screen}")
                self.screen.show_screen("PICKUP_MOVING")  # 기본값
        elif request.task_type_id == 2:  # 2: 호출
            self.get_logger().info("📞 호출 작업 카운트다운 완료")
            # TODO: 호출 관련 화면 추가 시 수정
            self.screen.show_screen("TOUCH_SCREEN")
        elif request.task_type_id == 3:  # 3: 길안내
